...
void main(void) {
  ...
  for(int y = -blur_value_int; y <= blur_value_int; y++) {
    ...
    vec3 sample = texture2D(adsk_results_pass10,current).rgb;
  ...
  if(energy > 0.0) {
    gl_FragColor = vec4((col/energy).rgb,texture2D(adsk_results_pass10,uv).a);
  } else {
    gl_FragColor = texture2D(adsk_results_pass10,uv);
  }
}
